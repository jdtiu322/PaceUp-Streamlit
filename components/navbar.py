...
from state import get_home_page, go_to, logout_user, toggle_theme
...
def render_nav() -> None:
    ...
    with st.container(key="top_nav"):
        left, mid, right, theme_col = st.columns([1.2, 1.45, 0.85, 0.22], gap="small")
        with left:
            if LOGO_IMAGE:
            ...
            if st.session_state.user:
                ...
                with b:
                    if st.button("Register", key="nav_register", type="primary", use_container_width=True):
                        go_to("register")
        with theme_col:
            with st.container(key="nav_theme_toggle"):
                label = "Light" if is_dark else "Dark"
                if st.button(label, key="nav_theme_btn", type="secondary", use_container_width=True, help="Toggle night mode"):
                    toggle_theme()
